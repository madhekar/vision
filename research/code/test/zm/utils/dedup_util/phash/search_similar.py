...
def search(df_dataset,output_path,tree_type,distance_metric,nearest_neighbors,leaf_size,parallel,batch_size, threshold,image_w, image_h,query=None,show=False):
    ...
    if df_image.empty:
        ...
        distances, indices = near_duplicate_image_finder.find_near_duplicates(
            image_id, nearest_neighbors, threshold
        )
        ...
        if len(distances) > 0 and len(indices) > 0:
            ...
            fig_acc.savefig(
                os.path.join(output_path, image_path.split(os.path.sep)[-1])
            )
            if show:
                plt.show()
            plt.cla()
