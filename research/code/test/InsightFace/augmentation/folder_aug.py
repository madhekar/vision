...
def apply_aug(faces_folder):
    for face in os.listdir(faces_folder):
        for filename in os.listdir(os.path.join(faces_folder, face)): 
            if filename.endswith(".jpg") or filename.endswith(".png"):                    
                # Avoid augmenting already augmented images
                if filename.startswith("aug_"):
                    continue
                for i in range(10):
                    file_path = os.path.join(faces_folder, face, filename)
                    ...
                    # Save in Same Folder
                    save_path = os.path.join(faces_folder, face, f"aug_{i}_{filename}")
...
if __name__=="__main__":
    faces_root = "/mnt/zmdata/home-media-app/data/app-data/static-metadata/faces/training/images"
    apply_aug(faces_root)