...
with st.container():
    ...
    st.divider()
    c0, c1, c2, c3, c4,c5 = st.columns ((1, 1, 1, 1, 1, 1), gap='small')
    with c0:
    ...
    with c1:  
      st.button("Duplicate Data Check", key="g1", on_click=btn_pressed_callback, args=(1,))
      st.divider()
      chart_data = pd.DataFrame(abs(np.random.randn(1, 4)) *100 , columns=["images", "text", "video", "audio"])
      st.bar_chart(chart_data, horizontal=False, stack=False, y_label="number of files", use_container_width=True)
      st.divider()
    ...
    with c2:  
       st.button("Data Quality Check", key="g2", on_click=btn_pressed_callback, args=(2,))
       st.divider()
       chart_data = pd.DataFrame(abs(np.random.randn(1, 4)) *100 , columns=["images", "text", "video", "audio"])
       st.bar_chart(chart_data, horizontal=False, stack=False , y_label="number of files", use_container_width=True)
       st.divider()
    ...
    with c4:         
       st.button("Metadata Correction", key="g4", on_click=btn_pressed_callback, args=(4,))
       st.divider()
       chart_data = pd.DataFrame(abs(np.random.randn(1, 4)) *100 , columns=["images", "text", "video", "audio"])
       st.bar_chart(chart_data, horizontal=False, stack=False, y_label="number of files", use_container_width=True)
       st.divider()
    ...
    with c5:     
       st.button("Data Loader Check", key="g5", on_click=btn_pressed_callback, args=(5,))
       st.divider()
       chart_data = pd.DataFrame(abs(np.random.randn(1, 4)) *100 , columns=["images", "text", "video", "audio"])
       st.bar_chart(chart_data, horizontal=False, stack=False, y_label="number of files", use_container_width=True)
       st.divider()
