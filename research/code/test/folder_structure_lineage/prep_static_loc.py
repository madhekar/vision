...
def init():
  ...
  df = pd.read_csv(os.path.join(zapp_static_locaton_prep_data_path, zapp_static_locaton_prep_file), delimiter=",")
...
def extract_location_details_with_error_chk(geolocator, df):
    ...
    for row in df.itertuples():
        ...
        try:
            ...
            pprint(
                f"error: geocode failed with {row.adress} with exception {e.message}"
            )
        time.sleep(2)
...
def prepare_static_metadata_locations(zapp_static_location_path, zapp_static_location_file):
    try:
        geo_locator, dfi = init()    
        ...
        create_default_location(
            zapp_static_location_path, zapp_static_location_file, parquet_schema, dfr
        )
    except Exception as e:
        pprint(f'error: exception in preparing static metadata {e.message}')

def create_default_location(
    static_location_path, static_location_file,schema, df
):
    table = pa.Table.from_pandas(df, schema=schema)
    try:
        pq.write_table(
            table, os.path.join(static_location_path, static_location_file)
...
if __name__=='__main__':
    prepare_static_metadata_locations()
    # sample output
