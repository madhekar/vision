...
class FolderStats:
    ...
    def count_file_types(self, directory):
    ...
    def get_all_image_types(self, file_counts, file_sizes):
        ...
        images_size = {key: self.get_size(file_sizes[key]) for key in image_types if file_sizes[key] > 0}
    ...
    def get_all_video_types(self, file_counts, file_sizes):
        ...
        videos_size = {key: self.get_size(file_sizes[key]) for key in video_types if file_sizes[key] > 0}
    ...
    def get_all_document_types(self, file_counts, file_sizes):
        ...
        documents_size = {key: self.get_size(file_sizes[key]) for key in document_types if file_sizes[key] > 0}
    ...
    def get_all_audio_types(self, file_counts, file_sizes):
        ...
        audios_size = {key: self.get_size(file_sizes[key]) for key in audio_types if file_sizes[key] > 0}
        return  audios_cnt, audios_size  

    def get_all_file_types(self, directory_path):        
        file_counts, file_sizes = self.count_file_types(directory_path)
...
if __name__ == '__main__':
   fstat = FolderStats()
   dfi, dfv,dfd,dfa = fstat.get_all_file_types('/home/madhekar/work/home-media-app/data/raw-data')
   if not dfi.empty:
     print(dfi.head())
