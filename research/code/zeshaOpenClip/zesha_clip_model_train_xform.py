...
class image_title_dataset:
    ...
    def __getitem__(self, idx):
        ...
        image = self.xform(Image.open(self.image_path[idx]))
...
text = torch.cat([clip.tokenize(f"a image of a {c}") for c in object_names]).to(device)
print(text.shape)
...
with torch.no_grad():
    ...
    image_features = model.encode_image(image).float() #convert to float32
